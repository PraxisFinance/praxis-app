...
import type { TechProbabilityChartPoint } from "@/shared/types/techHubEvent";
import { formatTechPoolPercent } from "@/shared/utils/techHubEventFormat";

const TECH_YES_LINE_COLOR = "#34c53e";

const chartConfig = {
  yesPercent: { label: "Yes", color: TECH_YES_LINE_COLOR },
} satisfies ChartConfig;

interface TechEventDetailProbabilityChartProps {
  points: TechProbabilityChartPoint[];
}

export function TechEventDetailProbabilityChart({ points }: TechEventDetailProbabilityChartProps) {
  const lastYesPercent = useMemo(() => {
    const last = points[points.length - 1];
    return last?.yesPercent ?? 0;
  }, [points]);

  const percentLabel = formatTechPoolPercent(lastYesPercent);
    ...
    <section className="bg-main-lightGray rounded-[10px] p-3">
      <div className="relative">
        ...
        <span
          ...
          style={{ color: TECH_YES_LINE_COLOR, top: `${percentLabelTopPx}px` }}
        >
          {percentLabel}
        </span>
        <ChartContainer config={chartConfig} className="h-[160px] w-full">
          <LineChart data={points} margin={{ top: 16, right: 4, bottom: 0, left: -8 }}>
            ...
            <Line
              type="monotone"
              dataKey="yesPercent"
              stroke={TECH_YES_LINE_COLOR}
              strokeWidth={2}
              dot={false}
              activeDot={{ r: 4, fill: TECH_YES_LINE_COLOR, strokeWidth: 0 }}
            />
          </LineChart>
        </ChartContainer>
      </div>
