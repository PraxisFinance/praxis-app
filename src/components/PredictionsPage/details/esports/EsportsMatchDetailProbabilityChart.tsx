...
import type { EsportsMatchProbabilityChartPoint, EsportsMatchTeam } from "@/shared/types/esportsMatch";
import {
  ESPORTS_DETAIL_TEAM1_COLOR,
  ESPORTS_DETAIL_TEAM2_COLOR,
} from "./esportsDetailTheme";

interface EsportsMatchDetailProbabilityChartProps {
  volumeLabel: string;
  points: EsportsMatchProbabilityChartPoint[];
...
export function EsportsMatchDetailProbabilityChart({
  volumeLabel,
  points,
  team1,
  team2,
}: EsportsMatchDetailProbabilityChartProps) {
  const { topTeam, bottomTeam } = useMemo(() => {
    const lastPoint = points[points.length - 1];
    if (!lastPoint) {
      return {
        topTeam: buildTeamSnapshot(team2.name, 0, ESPORTS_DETAIL_TEAM2_COLOR),
        bottomTeam: buildTeamSnapshot(team1.name, 0, ESPORTS_DETAIL_TEAM1_COLOR),
      };
    }
  ...
  const chartConfig = {
    team1Percent: { label: team1.name, color: ESPORTS_DETAIL_TEAM1_COLOR },
    team2Percent: { label: team2.name, color: ESPORTS_DETAIL_TEAM2_COLOR },
      ...
      <ChartContainer config={chartConfig} className="h-[160px] w-full">
        <LineChart data={points} margin={{ top: 4, right: 4, bottom: 0, left: -16 }}>
          ...
          <Line
            ...
            stroke={ESPORTS_DETAIL_TEAM1_COLOR}
            strokeWidth={2}
            dot={false}
            activeDot={false}
          />
          <Line
            ...
            stroke={ESPORTS_DETAIL_TEAM2_COLOR}
            strokeWidth={2}
            dot={false}
            activeDot={false}
          />
