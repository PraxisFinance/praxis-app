...
export function InviteFriendsPage() {
  ...
  const referralCode = INVITE_FRIENDS_MOCK_REFERRAL_CODE;
  const referrals = INVITE_FRIENDS_MOCK_FRIENDS;
    ...
    <div className="flex min-h-full flex-col gap-5 pb-8">
      ...
      <section className="flex flex-col gap-3">
        ...
        <div className="flex flex-row items-stretch gap-2">
          <Input
            readOnly
            value={referralCode ? `https://praxis.app/r/${referralCode}` : ""}
            placeholder="Your referral link"
            className={inputClassName}
          />
          <button type="button" className={actionButtonClassName} aria-label="Copy referral link">
        ...
        <div className="mt-1 flex flex-row items-stretch gap-2">
          <Input
            ...
            className={inputClassName}
          />
          <button type="button" className={actionButtonClassName} aria-label="Apply referral code">
      ...
      <section className="flex flex-col gap-3">
        <SectionHeader className="text-main-darkPurple">Your friends</SectionHeader>
          ...
          {referrals.map((friend) => (
            ...
            >
              ...
              <div className="text-main-darkPurple flex min-w-[5.75rem] max-w-[7.5rem] shrink-0 flex-row items-center justify-start gap-2 text-left text-sm font-medium tabular-nums leading-normal sm:min-w-[6.5rem] sm:text-base">
                ...
                <span className="min-w-0">{formatScore(friend.score)}</span>
              </div>
            </li>
          ))}
        </ul>
